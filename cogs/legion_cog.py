...
def _resolve_to_level3(address: str) -> str:
    """
    Given any hex address (level 1, 2, or 3), return a random level-3 address within it.
    - Level 1 (e.g. "G")     → picks random mid and inner: "G-<mid>-<inner>"
    - Level 2 (e.g. "G-3")   → picks random inner: "G-3-<inner>"
    - Level 3 (e.g. "G-3-2") → returned unchanged
    """
    ...
    return address  # already level 3
...
class LegionCog(commands.Cog):
    ...
    @app_commands.describe(
        ...
        address="Target hex address (level-3 preferred, e.g. G-3-2; coarser addresses pick randomly within)",
    )
    ...
    async def legion_spawn(self, interaction: discord.Interaction, address: str,
        ...
        # Resolve to a specific level-3 hex
        resolved = _resolve_to_level3(address)
